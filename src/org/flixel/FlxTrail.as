package org.flixel
{
	...
	{		
		 ...
		 */
		override public function FlxTrail(Sprite:FlxSprite, Image:Class, Length:int = 5, Delay:int = 3, Alpha:Number = 0.3, Diff:Number = 0.05):void
		{
			// Sync the vars 
			sprite = Sprite;
			delay = Delay;
		...
		override public function update():void
		{
			...
			if (counter >= delay && trailLength >= 1 && !freeze)
			{
				counter = 0;
		...
		public function increaseLength(amount:int):void
		{
			...
			{
				var trailSprite:FlxSprite = new FlxSprite(-100, 0, image);
				add(trailSprite);
				trailSprite.alpha = transp;
				transp -= difference;
				trailSprite.immovable = true;
